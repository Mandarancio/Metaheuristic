...
Particle::Particle(meta::RnSolution *position, double omega, double c1,
...
math::Vector<double> Particle::speed() { return speed_; }

double Particle::current_fitness() { return position_->fitness(); }

meta::RnSolution *Particle::current_position() { return position_; }

double Particle::best_fitness() { return particle_best_->fitness(); }

meta::RnSolution *Particle::best_position() { return particle_best_; }
...
PSO::PSO(meta::RnSolution *startSolution, int n_particle, int tmax,
         double omega, double c1, double c2, double vmax)
    : meta::AMeta(startSolution, "PSO"), omega_(omega), c1_(c1), c2_(c2),
      vmax_(vmax), n_particles_(n_particle), tmax_(tmax) {
  for (int i = 0; i < n_particle; i++) {
    Particle *p =
        new Particle(dynamic_cast<meta::RnSolution *>(startSolution->random()),
                     omega, c1, c2, vmax);
...
void PSO::reset(meta::ASolution *sol) {
  ...
  for (int i = 0; i < n_particles_; i++) {
    delete particles_[i];
    particles_[i] =
        new Particle(dynamic_cast<meta::RnSolution *>(sol->random()), omega_,
                     c1_, c2_, vmax_);
  }
}

std::vector<Particle *> PSO::particles() { return particles_; }
