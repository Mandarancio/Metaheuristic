...
private:
  _interpreter() {
    ...
    if (!s_python_function_show || !s_python_function_figure ||
        !s_python_function_plot || !s_python_function_subplot ||
        ...
        !s_python_function_save || !s_python_function_annotate ||
        !s_python_function_close || !s_python_function_arrow ||
        !s_python_function_contour || !s_python_function_imshow) {
...
bool contour(
    ...
    const std::vector<std::vector<Numeric>> &z) // dosn't work?? long n=10)
{
  ...
  // PyObject* res =
  // PyObject_Call(detail::_interpreter::get().s_python_function_contour, xyz,
  // kwargs);
  PyObject *res = PyObject_CallObject(
      detail::_interpreter::get().s_python_function_contour, xyz);
...
bool arrow(double x, double y, double dx, double dy, double h_width = 0.02,
           double h_length = 0.1) {
  ...
  PyObject *res = PyObject_Call(
      detail::_interpreter::get().s_python_function_arrow, xy, kwargs);
