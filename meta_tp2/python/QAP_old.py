...
class TabuMatrix:
    ...
    def is_mandatory(self,s: Solution,i:int, j:int):
        ti = self.__lt_matrix__[s[i]-1][j]+self.__max_history__
        tj = self.__lt_matrix__[s[j]-1][i]+self.__max_history__
        return self.__t__>ti  and self.__t__>tj
    ...
    def tabu(self,s: Solution,i: int, j:int):
        ...
        self.__lt_matrix__[s[i]-1][i]=self.__t__
        self.__lt_matrix__[s[j]-1][j]=self.__t__
    def t(self):
        return self.__t__
...
class QAP:
    ...
    def __perm_iteration__(self,s: Solution, current_fitness: int, best_fitness:int, diversification: bool, s_i: int, e_i: int ):
        min_f = sys.maxsize
        perm = [-1,-1]
        mandatory = False
        n=  self.n()
        for i in range(s_i,e_i):
            for j in range(i+1,s.n()):
                ...
                if f<min_f and f<best_fitness:
                    ...
                    if (t_mandatory or mandatory) and diversification:
    ...
    def __step_U__(self, s:Solution,current_fitness:int, best_fitness: int, diversification: bool):
        min_n = None
        min_f = sys.maxsize
        perm = [-1,-1]
        mandatory = False
    ...
    def recursive_U(self, s: Solution, current_fitness: int, best_f: int, best_s: Solution, steps: int = 50,diversification: bool=True):
        if steps <= 0 :
            return best_s, best_f
        s,current_fitness=self.__step_U__(s,current_fitness,best_f,diversification)
        if current_fitness<best_f:
            best_s = s
            best_f = current_fitness
        self.__is__.append(current_fitness)
        return self.U(s,current_fitness, best_f, best_s, steps - 1)

    def U(self, s0: Solution, iterations: int=1000, diversification: bool=True):
        s=s0
        f=self.fitness(s0)
        best_f = f
        best_n = s0
        for i in range(0,iterations):
            s,f=self.__step_U__(s,f,best_f,diversification)
